<template>
  <view class="page">
    ...
    <view v-else-if="orders.length === 0" class="state">暂无报修记录</view>
    <view v-else class="list">
      <view v-for="item in orders" :key="item.id" class="card" @click="goProgress(item.id)">
        <view class="line">
...
import { listMyRepairOrders, type WorkOrderItem } from '@/services/repair'
...
async function loadData() {
  ...
  error.value = ''
  try {
    orders.value = await listMyRepairOrders()
  } catch (e) {
...
function goProgress(id: string) {
  uni.navigateTo({ url: `/pages/repair/progress?id=${id}` })
}
