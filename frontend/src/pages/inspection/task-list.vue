<template>
  <view class="page">
    ...
    <template v-if="loading">
      <Skeleton variant="inspection-task-list" />
    </template>
    ...
    <view v-else class="task-list">
      <view v-for="item in list" :key="item.id" class="task-card">
        ...
        <view v-if="item.status === 'PENDING'" class="task-action" @tap="handleStart(item.id)">
          开始巡检
        </view>
      </view>
    </view>
...
async function fetchList() {
  loading.value = true;
  try {
    const res = await getInspectionTasks({ status: currentTab.value });
...
async function handleStart(id: string) {
  try {
    ...
    uni.showToast({ title: "已开始巡检", icon: "none" });
    fetchList();
  } catch (e: any) {
