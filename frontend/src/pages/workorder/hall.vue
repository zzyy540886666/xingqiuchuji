  ...
  <view class="page">
    ...
    <view v-else class="order-list">
      <view v-for="item in list" :key="item.id" class="order-card" @tap="goProcess(item.id)">
        ...
        <text class="order-location" v-if="item.location">{{ item.location }}</text>
...
async function fetchList() {
  loading.value = true;
  try { const res = await getWorkOrders({ status: currentTab.value }); list.value = res.items; } catch {} finally { loading.value = false; }
}

watch(currentTab, () => fetchList());
onShow(() => fetchList());

function goProcess(id: string) { uni.navigateTo({ url: `/pages/workorder/process?id=${id}` }); }
</script>
...
.priority { font-size: 20rpx; padding: 4rpx 12rpx; border-radius: 6rpx; }
.priority.HIGH, .priority.URGENT { background: #fee2e2; color: #dc2626; }
.priority.MEDIUM { background: #fef3c7; color: #92400e; }
.priority.LOW { background: #f3f4f6; color: #6b7280; }
.order-desc { display: block; font-size: 24rpx; color: #6b7280; margin-bottom: 6rpx; }
.order-location { display: block; font-size: 22rpx; color: #9ca3af; margin-bottom: 6rpx; }
